...
class BVHParser():
    ...
    def _read_offset(self, bvh, token_index):
        # print("bvh[token_index] in _read_offset: ", bvh[token_index])
    ...
    def _read_channels(self, bvh, token_index):
        ...
        # print("bvh[token_index] in _read_channels: ", bvh[token_index])
    ...
    def _parse_joint(self, bvh, token_index):
        ...
        token_index = token_index + 1
        joint_name = bvh[token_index][1] + ':' + bvh[token_index+1][1]
        token_index += 1
        ...
        if (joint_id == "End"):
            print("we set end_site to True")
            joint_name = parent_name + '_Nub'
            end_site = True
        joint = self._new_bone(parent_name, joint_name)
        #print("bvh[token_index-1]: ", bvh[token_index-1][0])
        open_brace = bvh[token_index][0]
        if open_brace != 'OPEN_BRACE':   # if bvh[token_index][0] != 'OPEN_BRACE':
            if joint_id != 'End':
                print('Was expecting brace, got ', open_brace)
                return None
            elif joint_id == 'End' and bvh[token_index-1][0] != 'OPEN_BRACE':
                print("Was expecting brace, got : ", bvh[token_index-1])
                token_index -= 1
                return None
        token_index = token_index + 1

        # print("token_index before reading next offsets: ", token_index)
        if end_site:
            token_index -= 1
        ...
        while (bvh[token_index][0] == 'IDENT' and bvh[token_index][1] == 'JOINT') or (bvh[token_index][0] == 'IDENT' and bvh[token_index][1] == 'End'):
            ...
            # print("token_index in while: ", token_index)
    ...
    def _parse_hierarchy(self, bvh):
        self.current_token = 0
        #print("bvh[self.current_token]: ", bvh[self.current_token])
        ...
        root_name = bvh[self.current_token][1]  # Skeleton name is Skeleton
        self.current_token = self.current_token + 1
        ...
        print("offsets, self.current_token: ", offsets, self.current_token)
        channels, self.current_token, order = self._read_channels(bvh, self.current_token)
        print("channels, self.current_token, order: ", channels, self.current_token, order)
